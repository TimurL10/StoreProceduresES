                       ...
                       FROM      nomenclature nn JOIN
                                         [ItemsCategory] c ON c.CategoryId = nn.categoryId CROSS apply
                                             ...
                                             (SELECT TOP 1 a.categoryName
                                              FROM      [ItemsCategory] a
                                              WHERE   a.categoryId = cc.parent_id) cp
                       WHERE   nn.categoryId = n.categoryId) ELSE '' END AS Param2, 'Кол-во в упаковке' + ' ' + cast(p.amountInPack AS nvarchar(10)) + '. ' + p.name AS Param3, techchar.techchar AS Param4, '' AS YaMCategory, '' AS GCategory, 
                  '' AS OzCategory
FROM     dbo.nomenclature AS n INNER JOIN
                  dbo.packs AS p ON p.nomenclatureId = n.id AND p.name LIKE '%малая%' INNER JOIN
                  dbo.barcode AS b ON b.nomenclatureId = n.id INNER JOIN
                  dbo.StockOfGoods AS s ON s.Id = n.id INNER JOIN
                  dbo.weight AS w ON w.nomenclatureId = n.id INNER JOIN
                  dbo.unit AS u ON u.weightId = w.weightId INNER JOIN
                  dbo.youtube y ON y.nomenclatureId = n.id /*Inner join attributes att ON att.nomenclatureId = n.id*/ INNER JOIN
                  dbo.unit AS uw ON uw.weightId = w.weightId CROSS apply
                      (SELECT TOP 1 nomenclatureId, concatLinks, link
                       FROM      images
                       WHERE   nomenclatureId = n.id) i CROSS apply
                      (SELECT substring
                                             ((SELECT a.name + ': ' + CASE value WHEN 'true' THEN 'да. ' ELSE value + c.measure + '. ' END
                                               ...
                                               WHERE   nomenclatureId = n.id AND value <> '' AND value <> 'false' FOR xml path('')), 1, 9999) AS techchar) techchar